    ...
    if(!$result)
    {
      ...
      $resultObj->Message = "Query Error";
        ...
        if ($row == 0) {
          ...
          if ($sqlType != "social" && $sqlType != "super") {
            $resultObj->ErrorCode = 206;
            $resultObj->Message = "Only Social can get State";
          }
          else {
            $sql = "SELECT state FROM user WHERE firstName=\"" . $FirstName . "\" AND lastName=\"" . $LastName . "\"";
            $result = $con->query($sql);
