...
if (!isset($_GET['APIKey']) || !isset($_GET['FirstName']) || !isset($_GET['LastName']) || !isset($_GET['Credit']) && !$fatalError)
{
          ...
          if ($sqlType != "bank" && $sqlType != "super") {
            ...
            if(!$result)
            {
              ...
              if ($row == 0) {
                ...
                try {
                  $sql = "UPDATE user SET credit = \"" . $Credit . "\", agency = \"" . $sqlName . "\", queryDate = \"" . date("Y-m-d h:i:sa"). "\"  WHERE firstName=\"" . $FirstName . "\" AND lastName=\"" . $LastName . "\"";
                  $con->exec($sql);
